...
export default function VaultPage() {
  ...
  // Enrich quotes with analysis-like scores
  const enriched = quotes.map(q => {
    ...
    // Calculate opportunity score based on multiple factors (no look-ahead)
  ...
  // Daily suggestions: high confluence, not trap flagged, underrated
  const { data: suggestionsData, isLoading: suggestionsLoading } = useQuery({
    queryKey: ['daily-suggestions'],
    queryFn: () => bursaApi.getDailySuggestions(
      enriched.map(e => ({ symbol: e.symbol, name: e.name, sector: e.sector, change: e.regularMarketChangePercent, volume: e.regularMarketVolume, volRatio: e.volRatio }))
    ),
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="px-4 pt-4">
      ...
      <p className="text-[10px] text-muted-foreground mb-4">AI-powered stock scanner, suggestions & trap detection</p>

      {/* Tab switcher */}
      <div className="flex gap-1 mb-4 bg-secondary rounded-lg p-1">
        {([
          { key: "scanner", label: "Opportunity Scanner" },
        ...
        <div className="space-y-2 pb-4">
          ...
          {!suggestionsLoading && (!suggestionsData?.suggestions || suggestionsData.suggestions.length === 0) && (
            <div className="glass-panel p-6 rounded-xl text-center">
              ...
              <p className="text-xs text-muted-foreground mt-2">No suggestions available yet</p>
            </div>
          )}
        </div>
      )}
